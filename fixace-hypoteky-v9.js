(() => {
  ...
  function calculate() {
    ...
    for (const id of inputs) $(id)?.removeAttribute('aria-invalid');
    if (errors.length) {
      const firstInvalid = [
        !(v.principal > 0) && 'principal',
        !(v.remainingYears >= 1 && v.remainingYears <= 50) && 'remainingYears',
        !(v.horizonYears >= 1 && v.horizonYears <= v.remainingYears) && 'horizonYears',
        (!(v.fixYearsA >= 1) || !(v.fixYearsA < v.fixYearsB)) && 'fixYearsA',
        (!(v.fixYearsB >= 1) || !(v.fixYearsA < v.fixYearsB)) && 'fixYearsB',
        ...['rateA','followRateA','rateB','followRateB'].filter((id) => !(v[id] >= 0 && v[id] <= 25))
      ].find(Boolean);
      if (firstInvalid && $(firstInvalid)) $(firstInvalid).setAttribute('aria-invalid', 'true');
      return;
    }
    ...
    const stressRate = v.followRateA + shock;
  ...
  function reset() {
    ...
    document.querySelectorAll('[data-scenario]').forEach((button) => {
      const active = button.dataset.scenario === 'flat';
      button.classList.toggle('active', active);
      button.setAttribute('aria-pressed', String(active));
    });
    setMode('basic');
