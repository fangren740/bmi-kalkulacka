...
(function (root, factory) {
  ...
  function calculate(v) {
    ...
    const rolls = [], strips = [], wallSummary = [];
    for (let i = 0; i < v.walls.length; i++) {
      ...
      for (let j = 0; j < count; j++) {
        ...
        for (const candidate of rolls) {
          const g = r ? mod(phase - candidate.used, r) : 0;
          if (candidate.used + g + length <= v.rollLength - allowance) { roll = candidate; gap = g; break; }
        }
        if (!roll) { roll = { number: rolls.length + 1, allowance, used: 0, cuts: [] }; rolls.push(roll); gap = phase; }
        const strip = { id: `${i + 1}.${j + 1}`, wall: i + 1, index: j + 1, phase, base, length, gap, start: roll.used + gap, roll: roll.number };
        roll.cuts.push(strip); strips.push(strip); roll.used += gap + length;
      }
    }
    rolls.forEach(roll => { roll.leftover = v.rollLength - allowance - roll.used; });
    return { state: 'valid', value: { input: v, rolls, strips, wallSummary,
