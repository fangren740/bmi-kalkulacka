(() => {
  ...
  function formatInput(id) {
    const input = $(id);
    if (!input) return;
    const decimals = ["shiftHours", "nightHours", "weekendHours", "holidayHours", "overtimeHours", "customHours", "nightPercent", "weekendPercent", "holidayPercent", "overtimePercent", "customPercent"].includes(id) ? 2 : 0;
  ...
  function setMode(mode, options = {}) {
    ...
    setText("formTitle", currentMode === "advanced" ? "Zkontrolujte směnu do detailu" : "Začněte rychlým výpočtem");
    setText("formLead", currentMode === "advanced"
      ...
      : "Vyberte způsob odměňování, zadejte sazbu a označte, kolik hodin bylo nočních, víkendových, svátečních nebo přesčasových.");
  ...
  function autoHours() {
    ...
    const toHours = (minutes) => Math.round(minutes / 15) / 4;
  ...
  function applyAutoHours() {
    ...
    if (!result) return;
    $("shiftHours").value = result.worked;
    $("nightHours").value = result.night;
    $("weekendHours").value = result.weekend;
    $("holidayHours").value = result.holiday;
    $("includeNight").checked = result.night > 0;
    $("includeWeekend").checked = result.weekend > 0;
    $("includeHoliday").checked = result.holiday > 0;
    ["shiftHours", "nightHours", "weekendHours", "holidayHours"].forEach(formatInput);
    run();
  }
