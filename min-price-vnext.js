(() => {
  ...
  function render() {
    ...
    // V-next hero blueprint mirrors the active calculation.
    setText('heroTarget',money(result.target));
    setText('heroBreakEven',money(result.breakEven));
    setText('heroList',money(result.catalog));
    const bpCost = document.querySelector('.mp-blueprint-track .cost');
    const bpFees = document.querySelector('.mp-blueprint-track .fees');
    const bpMargin = document.querySelector('.mp-blueprint-track .margin');
    if (bpCost && bpFees && bpMargin && result.target > 0) {
      bpCost.style.width = `${Math.max(0,Math.min(100,result.cost / result.target * 100))}%`;
      bpFees.style.width = `${Math.max(0,Math.min(100,values.fees * 100))}%`;
      bpMargin.style.width = `${Math.max(0,Math.min(100,values.margin * 100))}%`;
    }
  }
