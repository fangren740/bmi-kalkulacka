(function () {
  ...
  const elements = {
    ...
    proCurrentMonthlyCosts: $("proCurrentMonthlyCosts"), proNewMonthlyCosts: $("proNewMonthlyCosts"), proHoldingYears: $("proHoldingYears"), proStressDelta: $("proStressDelta"),
    proPrevStep: $("proPrevStep"), proNextStep: $("proNextStep"), proStepCounter: $("proStepCounter"),
    ...
    horizonSavingResult: $("horizonSavingResult"), horizonMeta: $("horizonMeta"), stressPaymentResult: $("stressPaymentResult"), stressDeltaResult: $("stressDeltaResult"),
    stressRate05Payment: $("stressRate05Payment"), stressRate10Payment: $("stressRate10Payment"), stressExtraCostsSaving: $("stressExtraCostsSaving"), sameTermSaving: $("sameTermSaving"),
  ...
  let latest = null;
  let proStep = 0;
  const proStepButtons = Array.from(document.querySelectorAll("[data-pro-step]"));
  const proPanels = Array.from(document.querySelectorAll("[data-pro-panel]"));
  ...
  function remainingBalance(principal, annualRate, months, paidMonths) {
    ...
    const k = Math.max(0, Math.min(months, Math.round(paidMonths)));
    if (k >= months) return 0;
    const monthlyRate = annualRate / 1200;
    if (monthlyRate === 0) return principal * (1 - k / months);
    const payment = annuity(principal, annualRate, months).payment;
    const factor = Math.pow(1 + monthlyRate, k);
    return Math.max(0, principal * factor - payment * (factor - 1) / monthlyRate);
  }

  function exitCost(scenario, horizonMonths) {
    const paid = Math.max(0, Math.min(scenario.months, Math.round(horizonMonths)));
    return scenario.oneTimeCosts + paid * scenario.monthlyOutflow + remainingBalance(scenario.principal, scenario.annualRate, scenario.months, paid);
  }
  ...
  function dataFromForm() {
    if (mode === "basic") {
      ...
      return {
        ...
        newMonthlyCosts: 0,
        horizonMonths: Math.min(Math.round(years * 12), 60),
        stressDelta: 1.5
      };
    }
    return {
      ...
      horizonMonths: Math.round(value(elements.proHoldingYears) * 12),
      stressDelta: value(elements.proStressDelta)
    };
  }
  ...
  function validate(data) {
    ...
    if (data.horizonMonths < 1 || data.horizonMonths > 600) return "Plánovaný horizont musí být alespoň 1 měsíc a nejvýše 50 let.";
    if (data.stressDelta < 0 || data.stressDelta > 10) return "Stresové zvýšení sazby musí být mezi 0 a 10 procentními body.";
    return "";
  }
  ...
  function render(data) {
    ...
    const stressScenario = model(data.principal, data.newRate + data.stressDelta, data.newMonths, data.newMonthlyCosts, data.oneTimeCosts);
    const stress05 = model(data.principal, data.newRate + 0.5, data.newMonths, data.newMonthlyCosts, data.oneTimeCosts);
    const stress10 = model(data.principal, data.newRate + 1, data.newMonths, data.newMonthlyCosts, data.oneTimeCosts);
    ...
    const horizonMonths = Math.min(Math.max(1, data.horizonMonths), Math.max(current.months, next.months));
    const currentHorizonCost = exitCost(current, horizonMonths);
    const nextHorizonCost = exitCost(next, horizonMonths);
    const horizonSaving = currentHorizonCost - nextHorizonCost;
    const sameTermNetSaving = current.total - fair.total;
    const maxInterest = Math.max(current.interest, next.interest, 1);
    const meaningful = Math.max(10000, data.principal * 0.005);

    latest = { data, current, next, fair, stressScenario, netSaving, interestSaving, monthlySaving, rateGap, termGap, breakEven, horizonSaving, horizonMonths, sameTermNetSaving };
    ...
    elements.horizonSavingResult.textContent = signedMoney(horizonSaving);
    elements.horizonMeta.textContent = "po " + monthLabel(horizonMonths) + " včetně zbývající jistiny";
    elements.stressPaymentResult.textContent = money(stressScenario.monthlyOutflow);
    elements.stressDeltaResult.textContent = "při sazbě " + percent(data.newRate + data.stressDelta);
    elements.stressRate05Payment.textContent = money(stress05.monthlyOutflow);
    elements.stressRate10Payment.textContent = money(stress10.monthlyOutflow);
    elements.stressExtraCostsSaving.textContent = signedMoney(netSaving - 20000);
    elements.sameTermSaving.textContent = signedMoney(sameTermNetSaving);
  ...
  function syncBasicToPro() {
    ...
    setValue(elements.proHoldingYears, Math.min(5, Math.max(0.5, months / 12)));
    setValue(elements.proStressDelta, 1.5);
  }
  ...
  function setMode(nextMode, shouldSync) {
    ...
    elements.formModeText.textContent = isBasic
      ...
      : "Nastavte současný úvěr, novou nabídku, úplné náklady a odolnost v přehledných čtyřech krocích.";
    if (!isBasic) setProStep(0);
    update();
  }

  function setProStep(index) {
    if (!proPanels.length) return;
    proStep = Math.max(0, Math.min(proPanels.length - 1, Number(index) || 0));
    proPanels.forEach((panel, i) => panel.classList.toggle("is-active", i === proStep));
    proStepButtons.forEach((button, i) => {
      button.classList.toggle("is-active", i === proStep);
      button.classList.toggle("is-done", i < proStep);
      button.setAttribute("aria-selected", String(i === proStep));
    });
    if (elements.proStepCounter) elements.proStepCounter.textContent = "Krok " + (proStep + 1) + " ze " + proPanels.length;
    if (elements.proPrevStep) elements.proPrevStep.disabled = proStep === 0;
    if (elements.proNextStep) elements.proNextStep.textContent = proStep === proPanels.length - 1 ? "Hotovo" : "Pokračovat";
  }
  ...
  function applyPreset(name) {
    if (name === "standard") {
      ...
      setValue(elements.proCurrentMonthlyCosts, 0); setValue(elements.proNewMonthlyCosts, 299); setValue(elements.proHoldingYears, 5); setValue(elements.proStressDelta, 1.5);
      ...
      setValue(elements.proCurrentMonthlyCosts, 0); setValue(elements.proNewMonthlyCosts, 0); setValue(elements.proHoldingYears, 5); setValue(elements.proStressDelta, 1.5);
    }
    update();
  }

  proStepButtons.forEach((button) => button.addEventListener("click", () => setProStep(button.dataset.proStep)));
  if (elements.proPrevStep) elements.proPrevStep.addEventListener("click", () => setProStep(proStep - 1));
  if (elements.proNextStep) elements.proNextStep.addEventListener("click", () => {
    if (proStep < proPanels.length - 1) setProStep(proStep + 1);
    else if (update()) $("vysledek").scrollIntoView({ behavior: "smooth", block: "start" });
  });
  ...
  elements.copyResult.addEventListener("click", async function () {
    ...
    const text = [
      ...
      "Úspora do horizontu " + monthLabel(latest.horizonMonths) + ": " + signedMoney(latest.horizonSaving),
      "Stresová nová splátka: " + money(latest.stressScenario.monthlyOutflow),
