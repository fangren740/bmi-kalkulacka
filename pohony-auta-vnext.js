...
function renderBreak(m){
 var status=$('breakStatus');var pairs=$('breakPairs');var chart=document.querySelector('.ptx-road-chart');var empty=$('breakEmpty');var paths={petrol:$('linePetrol'),diesel:$('lineDiesel'),ev:$('lineEv')};if(!m){Object.values(paths).forEach(function(p){if(p)p.setAttribute('d','')});if(pairs)pairs.replaceChildren();if(chart)chart.classList.remove('is-ready');if(empty)empty.hidden=false;if(status)status.innerHTML='<b>Zatím čeká na ceny aut</b><span>Rozbalte „Celé vlastnictví“ a zadejte pořizovací a prodejní cenu všech tří variant.</span>';drawGrid();return}
 if(chart)chart.classList.add('is-ready');if(empty)empty.hidden=true;if(status)status.innerHTML='<b>Živý model 5 000–50 000 km/rok</b><span>Čáry drží stejné ceny aut, stejné období a mění pouze roční nájezd. Průsečík je modelový bod zlomu, ne univerzální doporučení.</span>';
