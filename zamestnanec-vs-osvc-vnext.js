(() => {
  ...
  function renderBenchmarkMatrix() {
    const body = $('benchmarkMatrix');
    if (!body) return;
    const salaries = [40000, 60000, 80000, 100000, 120000, 150000];
    const costShares = [0.10, 0.20, 0.30, 0.40];
    const osvcBase = {
      monthlyCashCosts: 0,
      annualInvestments: 0,
      expenseMode: '60',
    ...
    body.innerHTML = salaries.map(gross => {
      const employee = calculateEmployee({ grossMonthly:gross, annualBonus:0, benefitsMonthly:0, benefitsAnnual:0, extraCredits:0, benefitValueShare:1 });
      const cells = costShares.map(share => {
        const monthlyCost = employee.monthlyCost * share;
        const osvc = calculateOsva(employee.employerCost, { ...osvcBase, monthlyCashCosts: monthlyCost });
        const delta = osvc.monthlyNet - employee.monthlyNet;
        const cls = delta > 5000 ? 'is-positive' : delta < 0 ? 'is-negative' : 'is-neutral';
        const sign = delta >= 0 ? '+' : '−';
        return `<td class="${cls}"><strong>${sign}${money(Math.abs(delta))}</strong><small>OSVČ − zaměstnanec / měs.</small></td>`;
      }).join('');
      return `<tr><td>${money(gross)}</td>${cells}</tr>`;
    }).join('');
  }
  ...
  function render() {
    ...
    $('heroRate').textContent = `${money(hourlyRate)}/h`;
    $('flowBudget') && ($('flowBudget').textContent = money(employee.monthlyCost));
    $('flowBudget2') && ($('flowBudget2').textContent = money(employee.monthlyCost));
    $('flowEmployee') && ($('flowEmployee').textContent = money(employee.monthlyNet));
    $('flowOsvc') && ($('flowOsvc').textContent = money(osvcSameBudget.monthlyNet));
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const menu = document.querySelector('.cvo-menu');
    const nav = document.querySelector('.cvo-nav');
    if (menu && nav) {
      ...
      document.addEventListener('keydown', event => {
        if (event.key === 'Escape') { nav.classList.remove('is-open'); menu.setAttribute('aria-expanded','false'); }
      });
    }

    const tabs = Array.from(document.querySelectorAll('.cvo-mode [role=tab]'));
